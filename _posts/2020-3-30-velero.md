...
Indeed, we can customize the backup method for each of applications, and that what we actually did in the past. The advantage of this approach is that we can ignore the low-level storage details while the obvious drawback is that the workload for specifying backup methods continues to increase as the number of applications grows, which makes it hard to maintain and automate the backup.      

So, we want to find a way to do `bur`(backup and restore) regardless of the type of application and [Kubernetes Persistent Volume](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#types-of-persistent-volumes), and velero comes into our sight.
...
# backup data in object storage provider still remains
...
`velero restic backup delete` will remove relevant backups, restores and disk snapshots，but it won't delete restic data and affect other backups
...
This article is more like a practical guide than theory analysis and stay tuned as this evolves.  
